                ...
                if (requestParams.TryGetValue("idp", out var idplist))
                {
                    ...
                    if (idp != null)
                    {
                        ...
                        if (clientRedirectUrl != null)
                        {
                            ...
                            var resourceHolderRedirectUrl = $"{context.Request.Scheme}://{context.Request.Host}{context.Request.PathBase}{TieredOAuthAuthenticationDefaults.CallbackPath}";
                            ...
                            if (idpClient != null)
                            {
                                idpClientId = idpClient.ClientSecrets.SingleOrDefault(cs => cs.Type == "TIERED_OAUTH_CLIENT_ID")?.Value;
                            }
                            ...
                            if (idpClient == null || !idpClient.Enabled || updateRegistration == "true")
                            {
                                ...
                                var communityId = await store.GetCommunityId(communityName, GetCancellationToken(context));

                                if (communityId == null)
                                {
                                    _logger.LogInformation("Tiered Oauth: Cannot find communityId for community: {communityName}", communityName);
                                ...
                                var document = await _udapClient.RegisterClient(
                                    ...
                                    _certificateStore.IssuedCertificates.Where(ic => ic.IdPBaseUrl == idp).Select(ic => ic.Certificate), 
                                ...
                                if (document.GrantTypes != null && document.GrantTypes.Contains(OidcConstants.GrantTypes.RefreshToken))
                                {
                                        ...
                                        client.AllowedGrantTypes.FirstOrDefault(t => t.Equals(GrantType.ClientCredentials)) != null)
                                    {
                                        ...
                                        await context.Response.WriteAsJsonAsync(error, cancellationToken: GetCancellationToken(context));
                            ...
                            var user = new TieredOAuthUser(idp)
                            {
                            ...
                            var props = new AuthenticationProperties
                            {
                                Items =
                                {
                                    new KeyValuePair<string, string?>(TieredOAuth.ClientRandomState, requestParams["state"]),
                                    new KeyValuePair<string, string?>(TieredOAuth.ResourceHolderRandomState, resourceHolderState),
