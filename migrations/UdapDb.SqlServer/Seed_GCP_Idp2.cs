...
public static class Seed_GCP_Idp2
{
    ...
    public static async Task<int> EnsureSeedData(string connectionString, string certStoreBasePath, ILogger logger)
    {
        ...
        sb.AppendLine("if not exists(select * from sys.server_principals where name = 'udap_Idp2')");
        sb.AppendLine("BEGIN");
        sb.AppendLine("CREATE LOGIN udap_Idp2 WITH PASSWORD = 'udap_password_idp2', DEFAULT_DATABASE =[Udap.Identity.Provider2.db], CHECK_EXPIRATION = OFF, CHECK_POLICY = OFF;");
        sb.AppendLine("END");
        sb.AppendLine("IF NOT EXISTS(SELECT principal_id FROM sys.database_principals WHERE name = 'udap_Idp2')");
        sb.AppendLine("BEGIN");
        sb.AppendLine("CREATE USER udap_Idp2 from LOGIN udap_Idp2;");
        sb.AppendLine("EXEC sp_addrolemember N'db_owner', N'udap_Idp2';");
        sb.AppendLine("END");
